...
class PLLPreProcessor():
    ...
    def __init__(
        ...
        self.Ds = Ds
        self.Os = Os
        self.max_f_pfd = self.vco_max/max(M)
        self.max_o = max(Os)
    
    def is_between(a, x, b):
        return min(a, b) <= x <= max(a, b)

    def __stability_metric__(self, f_pfd, f_vco, O):
        '''
        TODO check this
    ...
    def gen_pll_params( self, \
        ...
        vcos = [(M,D) for (M,D) in product(self.Ms, self.Ds) if is_between(vco_min, ref_clk * M / D, self.vco_max)]
        ...
        for (M, D) in vcos:
            for O in self.Os:
                ...
                for n in range (0, round(fgdiv * ref_clk * M / (D * out))):
                    ...
                    f_pfd = f_ref/M
                    f_vco = ref_clk * M / D
                    stability_metric = self.stability_metric(f_pfd, f_vco, O)
    ...
    def optimize_pll_configs(self, df, delay_min_ps, delay_max_ps, \
        ...
        if(metric == "stability"):
            for delay in range (delay_min, delay_max-res_ps, res_ps):
