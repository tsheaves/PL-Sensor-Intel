...
class TunableTDCSweeper():
    ...
    def __calc_pop_counts__(self, falling_samples, rising_samples, bitwidth):
        pop_counts_rising = [] 
        pop_counts_falling = []
        for sample_rise, sample_fall in zip(rising_samples, falling_samples):        
            pop_counts_rising.append(self.__popcount__(sample_rise))
            pop_counts_falling.append(bitwidth-self.__popcount__(sample_fall))
        return pop_counts_rising, pop_counts_falling

    def __decode_samples__(self, samples, max_val):
        '''
    ...
    def __firsts_lasts__(self, samples, pol, bitwidth):
        firsts = []
        lasts  = []
    ...
    def __calc_firsts_lasts__(falling_samples, rising_samples, bitwidth):
    ...
    def __process_pop_counts__(pop_counts_rising, pop_counts_falling):
    ...
    def __process_firsts_lasts__(firsts_rising, firsts_falling, lasts_rising, lasts_falling):
    ...
    def tune_theta(self, theta_cfg_params, phi_cfg_params,
        ...
        num_seq = floor(theta_samples/dma.max_transfer)
        delay_lower_bound = sweep_params[str(bitwidth)]["delay_lower_bound"]
        delay_upper_bound = sweep_params[str(bitwidth)]["delay_upper_bound"]
        target_mu         = sweep_params[str(bitwidth)]["target rising mu"]
        min_pd            = sweep_params[str(bitwidth)]["min var prop dist"]
        max_pd            = sweep_params[str(bitwidth)]["max var prop dist"]
            ...
            if(delay_ps > delay_lower_bound and delay_ps < delay_upper_bound):
                ...
                if check == 0:
                    ...
                    for i in range(n_seq):
                        ...
                        self.pulsegen.prime(0, n_samples)
                        # Collect TDC traces and append to samples
                        samples = samples + dma.get_dma_words(n_samples, int(self.tdc_len/8))
                    ...
                    falling_samples, rising_samples \
                        = self.__decode_samples__(samples, 2**bitwidth-1)
                    if(debug):
                    ...
                    pop_counts_rising, pop_counts_falling \
                        = self.__calc_pop_counts__( falling_samples, rising_samples, bitwidth )
                    if(debug):
                        for i in range(int(len(samples)/4)):
